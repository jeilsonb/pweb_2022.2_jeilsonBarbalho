...
import br.com.jeilson.biritashop.model.dto.ItemPedidoDTO;
import br.com.jeilson.biritashop.model.dto.PedidoDTO;
import br.com.jeilson.biritashop.model.enums.FormaPagamento;
import br.com.jeilson.biritashop.repository.ClienteRepository;
import br.com.jeilson.biritashop.repository.ItemPedidoRepository;
...
public class ItemPedidoController {
    ...
    public ModelAndView compras(PedidoDTO pedidoDTO) {
        Cliente cliente = this.clienteRepository.findById(pedidoDTO.getCliente().getId()).orElseThrow(() -> new IllegalArgumentException("ID Inválido " + pedidoDTO.getCliente().getId()));
    ...
    public List<ItemPedido> converter(Pedido pedido, List<ItemPedidoDTO> itemPedidos) {
        ...
        return filtrado.stream().map(dto -> {
            Produto temp = this.produtoRepository.findById(dto.getId()).orElseThrow(() -> new IllegalArgumentException("ID Inválido " + dto.getId()));
            ItemPedido itemPedido = ItemPedido.builder().quantidade(dto.getQuantidade()).valorUnitario(temp.getPrecoVenda()).produto(temp).pedido(pedido).build();
            return itemPedido;
